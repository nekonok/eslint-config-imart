  ...
  return Q.all(
    _.keys(apiMap).map((filename) => {
      const content =
        `${'module.exports = {\n' +
        '  "globals": {\n'}${
        apiMap[filename].sort().map(k => `    "${k}": false`).join(', \n')}\n` +
        '  }\n' +
        '};';
...
function requestAndMapPromises() {
  const tasks =
  parsers.map(elm => elm.targets.map(t => client.fetch(t.url).then(elm.parse).then(appendApi.bind({ file: t.file }))
    ));
...
.then(files => Q.all(files.map(file => Q.nfcall(fs.unlink, `globals/${file}`))))
